...
if __name__ == "__main__":
    ...
    from feature_extraction import prep_data, _median_transform
    ...
    outdir = Path(f"../features/median-features/{args.n_dims}d/")
    outdir.mkdir(exist_ok=True, parents=True)

    logger = get_logger(name="basic", tag=f"median_features/{args.n_dims}d")
    ...
    for i in range(nchunk):
        ...
        with timer(f"chunk{i+1}", logger):
            X_temp = prep_data(args.path, _median_transform, current_head,
                               step, args.n_dims)
            X.append(X_temp)
            current_head += step
