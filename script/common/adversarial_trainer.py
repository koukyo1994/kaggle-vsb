        ...
        for epoch in range(n_epochs):
            with timer(f"Epoch {epoch+1}/{n_epochs}", self.logger):
                ...
                for (x_batch, y_batch) in train_loader:
                    ...
                    for (x_batch_loc, y_batch_loc) in self.local_loader:
                        y_pred_loc = model(x_batch_loc)
                        loss_loc = self.loss_fn(y_pred_loc, y_batch_loc)
                        loss += loss_loc * self.loc_lambda
                    optimizer.zero_grad()
