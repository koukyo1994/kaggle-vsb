...
if __name__ == "__main__":
    ...
    from script.augmentation.augmentors import VerticalShufflingAugmentor
    ...
    parser.add_argument("--n_shuffle", default=1, type=int)
    ...
    logger.info(f"n_shuffle: {args.n_shuffle}")
    ...
    vsa = VerticalShufflingAugmentor(
        path=args.features[0], meta_path=args.metadata[0], support_dict={})
    train_new, answer_new = vsa.augment(
        n_pos=50000, n_neg=50000, n_shuffle=args.n_shuffle, seed=args.seed)

    train = np.vstack([train, train_new])
    answer = np.concatenate([answer, answer_new])
